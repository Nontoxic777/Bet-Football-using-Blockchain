...
contract BetFootball {
    ...
    uint256 private constant MaximumBet = 5e17; // 0.5 ETH
    ...
    mapping(string => uint256) s_betNameList;
    ...
    function addNewBetPair(
        ...
        if (pairId == 0) revert BetFootBall__IDPairAlreadyCreated();
        if (bytes(s_betPairList[pairId]).length != 0)
            revert BetFootBall__IDPairAlreadyCreated();
        if (s_betNameList[name] > 0) revert BetFootBall__IDPairAlreadyCreated();
        s_betPairList[pairId] = name;
        s_betNameList[name] = pairId;
    }

    function removeBetPairId(uint256 pairId) public onlyOwner {
        if (bytes(s_betPairList[pairId]).length == 0)
            revert BetFootBall__IDPairNotFound();
        delete s_betPairList[pairId];
        delete s_betNameList[s_betPairList[pairId]];
    }
    ...
    function getPairId(string memory name) public view returns (uint256) {
        return s_betNameList[name];
    }
    ...
    function getBetMinAmount() public pure returns (uint256) {
        return MinimumBet;
    }
